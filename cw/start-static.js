// CW 2.4.3 
// Static hosting login and system start
// (c) 2019 ermouth

(function(){

  // Mount CloudWall static starter
  // as cw.login

  cw.login = function _startAsStatic(){

    var isS = Object.isString, 
        isO = Object.isObject, 
        isA = Object.isArray,
        loc = window.location,
        hasLocalProfile = false,
        db,
        needLogout = false;

    var loc = window.location,
        root = loc.protocol + '//' + loc.hostname + (loc.port ? (':' + loc.port) : '') + '/';
    
    cw.root = root;
    cw.crypto.locale($.my.locale());

    return _checkLocalProfileStatic()
    .then(_setUpProfileStatic)
    .then(_startCW)
    .fail(function(code, msg, e){

      // Logout
      if (needLogout) _req('_session', {
        method:'POST', dataType:'json',
        headers:{'Content-type':'application/json'},
        data:JSON.stringify({name:'0', password:'0'})
      });

      // Write error
      $('#cw-page #cw-space').html(
        '<div class="dib vat p100 red1 tac">CloudWall can’t start. ' 
        + (msg+'').escapeHTML()
        +'</div>'
      );
      console.log("=== CloudWall failed to start ===", code, msg, e);
    });

    // - - - - - - - - - 

    function _checkLocalProfileStatic() {
      // System DB and profile
      var pi = $.Deferred();

      cw.profile().then(function(profile){
        cw.userCtx = {name:profile.name, roles:['_admin']};
        hasLocalProfile = true;
        pi.resolve();
      })
      .fail(function(){ pi.resolve(); });

      return pi.promise();
    }

    // - - - - - - - - -

    function _setUpProfileStatic(){
      // If no local profile,
      // creates it from default
      var pi = $.Deferred();
      
      if (hasLocalProfile) pi.resolve();
      else {
        _req('', {url:cw.CW.profileTemplate})
        .then(function(res){
          res.stamp = Date.now();
          res.name = (cw.userCtx||{}).name||'';
          res._id = 'cw';
          res.keys.forEach(function(key){
            if (!key.id) {
              key.id = cw.lib.hash8(Number.random(-1e13,1e13)+Date.now()+"");
              key.key = cw.lib.key16(Number.random(-1e13,1e13));
              key.stamp = Date.now();
            }
          });
          res.dbs.forEach(function(db){ if (!db.stamp) db.stamp = Date.now(); });

          return $('#cw-space').my(
            {
  "id": "cw.Sys.Profile.Create",
  "build": 87,
  "params": {
    "animate": 100,
    "effect": function ($e,onoff,duration) {
            if (onoff) { $e.slideDown(duration); return; }
            $e.slideUp(duration);
          
      }
  },
  "error": "Could not decrypt settings file",
  "Dbs": ["sysmanual"],
  "ui": {
    "#hheader": function (d) {
          var that = this, 
              cfg = cw.config || function(k){
                return ({title:'CloudWall', titleColor:'#60a9e6', logo:'logo.svg'})[k];
              };
          return [
            '<div class="dib vat w100p nw oh">',
              '<img class="hub-logo" src="'+cfg('logo')+'">',
              '<div class="hub-title" style="color:'+cfg('titleColor')+';">',
                (cfg('title') || 'CloudWall').escapeHTML(),
              '</div>',
            '</div>'
          ].join('');
        
      },
    "#dburl": {
      "bind": function (d,v,$o) {
              var a;
              if (v!=null) {
                if (a = v.match(/^http[s]?:\/\/([a-z0-9\-]{1,63}:[^@]{1,63}@)/)) {
                  d.dburl = v.replace(a[1],'');
                  d.dblogin = a[1].split(":")[0];
                  d.dbpwd = a[1].split(":")[1].to(-1);
                }
                else {
                  d.dburl = v;
                }
              }
              return d.dburl;
            
        },
      "check": function (d,v) {
              if (!v.length) return "";
              if (!/^http[s]?:\/\/.+/.test(v)) return "URL required";
              if (!/^http[s]?:\/\/([a-z0-9\-]{1,63}:[^@]{1,63}@)?[a-z0-9\-]{1,63}(\.[a-z0-9\-]{1,63}){1,5}/.test(v)) 
                return "Invalid domain";
              if (!/.+\/[a-zA-Z0-9_\-\(\)]{1,200}$/.test(v)) return "No DB name provided";
              return "";
            
        }
    },
    "#dblogin": {
      "bind": "dblogin",
      "watch": "#dburl"
    },
    "#dbpwd": {
      "bind": "dbpwd",
      "watch": "#dburl"
    },
    "#btn-connect": {
      "delay": 50,
      "bind": function (d,v,$o) {
              var that = this,
                  db = this.db, 
                  edb,
                  url, 
                  creds = "";
              if (v!=null && $o.my().root.my('valid') ) {
                if (!d.dburl) return;
                if (d.dbpwd+d.dblogin && (!d.dbpwd || !d.dblogin)) {
                  cw.lib.note("Both login and password required.");
                  return;
                }
                creds = d.dblogin?d.dblogin+":"+d.dbpwd+"@":"";
                url = d.dburl.replace(/^(http[s]?:\/\/)(.+)$/,"$1"+creds+"$2");
                console.log("Trying to connect to the remote DB...")
                //try to connect
                edb = new this.Pouch(url, function (e,res) {
                  var msg;
                  if (e) {
                    msg=e.message;
                    console.log(e)
                    if (!msg) _fail("Can’t connect to DB.");
                    else if (msg==="missing") _fail("DB does not exist.");
                    else if (/not\sauth/.test(msg)) _fail("Authorization required.");
                    else if (/or\spass/.test(msg)) _fail("Name or password are incorrect.");
                    else _fail("Error connecting to DB.")
                  }
                  else {
                    edb.get("cw", function (e,res) {
                      var msg;
                      if (e) {
                        msg=e.message;
                        if (!msg) _fail("Can’t connect to DB.");
                        else if (msg==="missing") _fail("DB or account does not exist.");
                        else if (/not\sauth/.test(msg)) _fail("Invalid credentials.");
                        else _fail("Error reading account.");
                      } else {
                        console.log("Read remote cw", res)
                        _process(res);
                      }
                    });
                  }
                });
              }
              function _process (doc){
                if (!d.isnew && d.doc) {
                  //delete all dbs
                  var dbs = d.doc.dbs.map(function(e){return e.name});
                  console.log(dbs);
                  cw.crypto.init(doc)
                    .then(function(){
                    //delete dbs
                    $.my.modal({
                      manifest:that.Confirm,
                      data:{
                        text:"Delete local DBs "+dbs.join(", ")+"?",
                        ok:"Proceed"
                      }
                    })
                      .then(function(r){
                      var db,
                          d1 = that.Settings(),
                          d2 = cw.crypto.enc(d1);
                      var _done = (function(){
                        _create();
                      }).after(dbs.length);
                      if (Object.isObject(r)) {
                        dbs.forEach(function(e){
                          that.Pouch.destroy(e, _done);
                        });
                      } else _fail("Cancelled.");
                      function _create(){
                        console.log("Creating account doc...")
                        db = new that.Pouch("cw", function(){                    
                          $.my.modal(
                            '<div class=" pt20 pb15 w350 tac blue">'
                            +'Reloading account, it can take a time. '
                            +'Please, wait.'
                            +'<br><div class="cw-busy w50 mt10"></div>'
                            +'</div>'
                            ,function(){},500);
                          db.replicate.from (url, {
                            complete:function (e, res){
                              console.log("Replication ", e,res);
                              window.location.reload();
                            }
                          });
                        });
                      }
                    })
                      .fail(function(){
                      _fail("Cancelled.")
                    });
                  })
                    .fail(function(){
                    _fail("Invalid PIN for received account.")
                  });
                } else {
                  //load doc and save
                  db = new that.Pouch("cw", function(){
                    $.my.modal(
                      '<div class=" pt20 pb15 w350 tac blue">'
                      +'Loading account, it can take a time. '
                      +'Please, wait.'
                      +'<br><div class="cw-busy w50 mt10"></div>'
                      +'</div>'
                      ,function(){},500);
                    db.replicate.from (url, {
                      complete:function (e, res){
                        console.log("Replication ", e,res);
                        window.location.reload();
                      }
                    });
                  });
                }
              }
              function _fail(msg){
                cw.lib.note(msg, "error");
              }
            
        },
      "events": "click.my"
    },
    "#aboutLink": {
      "bind": function () {
              return this.lang.IntroExt;
            
        }
    },
    "#btn-logout": {
      "bind": function (d,v) {
              if (v==null) return;
              $.ajax('/_session', {
                method:'POST', dataType:'json',
                headers:{'Content-type':'application/json'},
                data:JSON.stringify({name:'0', password:'0'})
              })
              .always(function(){
                location.reload();
              });
            
        },
      "css": {
        "self:hide": function (d) {
                  return !d.iscouch
                
          }
      }
    },
    "#pic": {
      "bind": function (d,v,$o) {
              if (v!=null) {
                $.my.modal({
                  manifest:this.Cropper,
                  data:{data:"",filename:""}
                }).then(function (crop) {
                  if (crop && crop.data) {
                    d.doc.pic="data:image/jpeg;base64,"+crop.data;
                    $o.trigger("recalc");
                    crop.data="";
                  }
                });
              }
              return d.doc.pic;
            
        },
      "events": "click.my"
    },
    "#pics": {
      "init": function ($o,form) {
              var files = this.files,
                  t = '';
              for (var i=1;i<5;i++) {
                var pic = files[i+'.png'],
                    url = 'data:'+pic.content_type+';base64,'+pic.data;
                t+='<img src="'+url+'" class="hw28 mr2 icosmall br2 cp" />';
              }
              t+=[
                '<button class="hw28 mt0 mb0 mr0 ml2 button br2 pl0 pr0 dib vat blue fs80 o90" '
                  +'id="btn-addpic"  title = "Add custom image">',
                  '<span class="fi-photo"></span>',
                '</button>'
              ].join("");
              $o.html(t);
              $o.on("click",'.icosmall', function(){
                var im = $(this),
                    url = $(this).attr("src"),
                    pic = $o.my("find","#pic");
                im.attr("src", pic.attr("src"));
                pic.attr("src", url);
                form.data.doc.pic = url;
              });
            
        }
    },
    "#btn-addpic": {
      "bind": function (d,v,$o) {
              var that = this;
              if (v!=null) {
                $.my.modal({
                  manifest:that.Cropper,
                  data:{data:"",filename:""}
                }).then(function (crop) {
                  if (crop && crop.data) {
                    d.doc.pic="data:image/jpeg;base64,"+crop.data;
                    $o.my("find","#pic").trigger("recalc");
                    crop.data="";
                  }
                });
              }
            
        },
      "events": "click.my"
    },
    "#name": {
      "bind": "doc.name",
      "check": /^(|[a-z0-9]{3,40})$/i,
      "error": "3–40 latins and nums",
      "css": {
        ":disabled": function (d) {
          return !d.isnew || !!cw.userCtx
          },
        "self:bg-gray6 xgray": function (d) {
          return !d.isnew || !!cw.userCtx
          }
      },
      "recalc": "#pin"
    },
    "#pin": {
      "bind": "pin"
    },
    "#btn-create": {
      "delay": 50,
      "bind": function (d,v,$o) {
              var that = this, 
                  i, db, d2, 
                  uname = d.doc.name+'-'+d.doc.uid,
                  L = that.lang;
              if (v!=null && $o.my().root.my('valid') && this.db) {
                if (cw.lib.sdbm($o.my('find','#pic').attr('src'))=='79adusu9') {
                  cw.lib.note(L.ERR_UPIC, 'error');
                }
                else {
                  if (d.isnew) {
                    d.doc.pin = _pin();
                    d.doc.dbs[0].creator = uname;
                    d.doc.dbs = d.doc.dbs.compact(true);
                    d.doc.keys = d.doc.keys.map(function(e){
                      e.emitter = uname; 
                      e.link = d.doc.contact;
                      return e;
                    });
                  } 
                  else if (d.pin) d.doc.pin = _pin();
                  //console.log(d.doc);
                  try{cw.crypto.enc(d.doc)}
                  catch(e) {console.log(e.message, e.stack, e);}
                  if (d.isnew) {              
                    $.my.modal(
                      '<div class=" pt20 pb15 w350 tac blue">'
                      +L.MSG_LOADING_APPS
                      +'<br><div class="cw-busy w50 mt10"></div>'
                      +'</div>'
                      ,function(){},500);
                    // Read app list
                    $.ajax({ url:(cw.CW?cw.CW.appUpdateURL:'')||'apps.json', type:'GET', dataType:'json', cache:false })
                    .then(function(a){
                      var b, i, j, o;
                      if (!Object.isArray(a) || !a.length) {
                        $.my.modal();
                        cw.lib.note(L.ERR_LOADING_APPS, 'error', 5000);
                        console.log(a);
                      }
                      else {
                        b = Object.clone(a.filter(function(e){return Object.isObject(e)}), !0);
                        for (var i=0;i<b.length;i++) {
                          o=b[i];
                          o.srcrev = o._rev;
                          delete o._rev;
                          if (o._attachments) for (j in o._attachments) {
                            o._attachments[j] = Object.select(o._attachments[j],["content_type","data"]);
                          }
                        }
                        // encrypt settings doc
                        b.push(cw.crypto.enc(d.doc));
                        that.db.bulkDocs({docs:b})
                        .then(
                          function(a){
                            // everything is ok, reload page  
                            $.my.modal();
                            cw.lib.note(L.MSG_ACCOUNT_CREATED, 'ok', 5000);
                            var hash = (window.location.hash || '').replace(/^#/,'');
                            window.location.reload();
                          },
                          function(a){
                            $.my.modal();
                            cw.lib.note(L.ERR_LOADING_APPS, 'error', 5000);
                            console.log(a);
                          }
                        )
                      }
                    })
                    .fail(function(e){
                      $.my.modal();
                      cw.lib.note(L.ERR_LOADING_APPS, 'error', 5000);
                      console.log(e);
                    });
                  } else {
                    this.db.put(cw.crypto.enc(d.doc), function (){
                      window.location.reload();
                    });
                  }
                }
              }
              function _pin() {
                return cw.lib.md5(cw.lib.md5(cw.lib.md5(d.pin)+cw.lib.hash8(d.pin))+d.doc.name);
              }
            
        },
      "events": "click.my",
      "watch": "#name",
      "css": {
        ":disabled": function (d) {
          return !d.doc.name || !/^(|[a-z0-9]{3,40})$/i.test(d.doc.name);
          }
      }
    }
  },
  "data": {
    "isnew": true,
    "iscouch": false,
    "dburl": "",
    "dbpwd": "",
    "dblogin": "",
    "prevpin": "",
    "pin": "",
    "doc": {
    }
  },
  "lang": {
    "en": {
      "MSG_LOADING_APPS": "Loading system apps, please wait a minute.",
      "ERR_LOADING_APPS": "Server responded with an error. Please retry in 5-10 minutes.",
      "ERR_UPIC": "Please change userpic",
      "PLC_LOGIN": "Login",
      "TIP_LOGIN": "Can not be changed later, be careful!",
      "PLC_PIN": "PIN code",
      "TIP_PIN": "Every time you log in \nCloudWall will ask you to enter PIN.\nLeave blank to skip PIN reenter.",
      "PLC_DBURL": "CouchDB bucket, that holds your account",
      "PLC_DBLOGIN": "DB login",
      "PLC_DBPWD": "DB password",
      "BTN_CREATE": "Create profile",
      "BTN_CONNECT": "Connect",
      "BTN_NEWU": "Or create new...",
      "BTN_LOGOUT": "Log out",
      "MSG_ACCOUNT_CREATED": "Account created, reloading page.",
      "ERR_ACCOUNT_FAIL": "Saving account info and apps failed. Try again, please.",
      "Intro": "<h3 class=\"mt0 xgray\">Create local profile</h3> \n<p class=\"lh170 mb5\">\n  No user profile found on this device.\n  Please, provide data for creating new local account.\n</p>\n<p class=\"lh170\">\n  Make your PIN short – CloudWall requests it on each start.\n</p>\n\n<h3 class=\"xgray\">Link to remote profile</h3>\n<p class=\"lh170\">\n  If you already have CouchDB, that holds your CloudWall account,\n  you can <span class=\"pseudolink btn-link\">link this device</span> \n  with your existing profile.\n</p>",
      "IntroExt": "<p class=\"lh170 mt-5\">\n  Connecting to external DB will sync this browser’s local databases \n  with external account. Sync destroys all existing local \n  CloudWall DBs if any – with no rollback possible.\n</p>\n<p class=\"\">\n  Please note that full account sync may take significant time.\n</p>",
      "IntroCouch": "<h3 class=\"mt0 xgray\">Profile set up</h3> \n<p class=\"lh170 mb5\">\n  Seems you’re starting CloudWall first time on current domain. \n  No user profile found on this device, remote profile is also empty. \n</p>\n<p class=\"lh170\">\n  To set up your account properly, please provide a PIN code to encrypt you sensitive profile data. \n  Consider not making your PIN excessively complex, CloudWall requests it on each start.\n</p>\n<p class=\"lh170\">\n  Changing default user icon is not required, however if you have a reasonable image, please upload it.\n</p>"
    },
    "ru": {
      "MSG_LOADING_APPS": "Загрузка системных приложений, подождите немного.",
      "ERR_LOADING_APPS": "Сервер ответил ошибкой. Попробуйте позже или обратитесь к админу.",
      "ERR_UPIC": "Смените юзерпик",
      "PLC_LOGIN": "Логин",
      "TIP_LOGIN": "Будет нельзя сменить позже, проверьте!",
      "PLC_PIN": "PIN код",
      "TIP_PIN": "Этим кодом шифруется ваш профиль и система будет спрашивать его при каждой перезагрузке.",
      "PLC_DBURL": "База CouchDB хранящая ваш эккаунт",
      "PLC_DBLOGIN": "Логин в БД",
      "PLC_DBPWD": "Пароль БД",
      "BTN_CREATE": "Создать",
      "BTN_CONNECT": "Подключить",
      "BTN_NEWU": "Или создать новый...",
      "BTN_LOGOUT": "Выход",
      "MSG_ACCOUNT_CREATED": "Эккаунт создан, перезагрузка страницы.",
      "ERR_ACCOUNT_FAIL": "Сохранение профиля не удалось, попробуйте позже или обратитесь к админу.",
      "Intro": "<h3 class=\"mt0 xgray\">Создать профиль</h3> \n<p class=\"lh170 mb5\">\n  На этом устройстве не найден профиль пользователя.\n  Нужно указать данные для создания такого профиля.\n</p>\n<p class=\"lh170\">\n  PIN код не нужно делать слишком длинным, система запрашивает его при каждом старте.\n</p>\n\n<h3 class=\"xgray\">Подключить профиль</h3>\n<p class=\"lh170\">\n  Если у вас БД CouchDB, содержащая ваш профиль,\n  вы можете <span class=\"pseudolink btn-link\">подключить</span> \n  это устройство к имеющемуся профилю.\n</p>",
      "IntroExt": "<p class=\"lh170 mt-5\">\n  Подключение к внешнему эккаунту синхронизирует локальные БД\n  с внешними источниками. Это удалит все уже имеющиеся локальные данные.\n</p>\n<p class=\"\">\n  Полная синхронизация с новым эккаунтом может занять существенное время \n  и создать большой поток запростов и трафика.\n</p>",
      "IntroCouch": "<h3 class=\"mt0 xgray\">Создание профиля</h3> \n<p class=\"lh170 mb5\">\n  Кажется, вы запускаете систему впервые на этом домене. \n  Профиль пользователя не найден ни на этом устройстве, ни в удалённой БД.\n</p>\n<p class=\"lh170\">\n  Для того, чтобы защитить ваш эккаунт, придумайте PIN код для шифрования данных вашего профиля.\n  PIN код не обязательно делать длинным, он запрашивается при каждом запуске системы как дополнительная мера безопасности.\n</p>\n<p class=\"lh170\">\n  Также лучше сразу загрузить новый аватар.\n</p>"
    }
  },
  "init": function ($o) {
      var db, that = this, 
          pi = $.Deferred(), 
          _isnew=true, 
          _settings = cw.crypto._settings,
          d = that.data.doc;
      
      var pic = this.files['0.png'];
      d.pic = 'data:'+pic.content_type+';base64,'+pic.data;
      that.Pouch = PouchDB;
      that.Settings = _settings;
      that.data.isnew = _isnew;
      db = PouchDB('cw');
      _new({});
      
      $o.on('click.my','.btn-link', function(){
        $o.find('.slide').slideToggle(200);
      });
      
      return pi.promise();
      //- - - - - - - - - - - - - - - - - 
      function _new(doc){
        $.extend(true, that.data.doc, doc);
        that.db = db;
        if (!that.data.doc.uid) that.data.doc.uid=cw.lib.hash8(Date.now()+Number.random(1e30));
        $o.html($.my.formgen(that.HTML).assign(that.lang));
        $o.find('#abouttext').html(that.data.iscouch?that.lang.IntroCouch:that.lang.Intro);
        pi.resolve();
      }
    },
  "Confirm": {
    "id": "cw.Sys.Confirm",
    "params": {
      "delay": 20,
      "strict": true,
      "width": 350
    },
    "data": {
      "text": "",
      "css": "xgray",
      "ok": "Ok",
      "cancel": "Cancel"
    },
    "init": function ($o,form) {
          var d=form.data;
          $o.formgen([
            '<div class="'+d.css+'">'+d.text+'</div>',
            {label:"70px", row:"350px", rowCss:"my-row mt15 pt15 fs90 mb-5 xgray btd"},
            ['', 'btn#btn-ok.green.mr5',{val:d.ok}, 
             'btn#btn-cancel',{val:d.cancel}]
          ]);
        
      },
    "ui": {
      "#btn-ok": {
        "bind": function (d,v,$o) {
                  if (v!=null) {
                    $o.trigger("commit");
                  }
                
          },
        "events": "click"
      },
      "#btn-cancel": {
        "bind": function (d,v,$o) {
                  if (v!=null) {
                    $o.trigger("cancel");
                  }
                
          },
        "events": "click"
      }
    }
  },
  "Cropper": {
    "params": {
      "strict": true,
      "width": 770
    },
    "data": {
      "filename": "",
      "data": "",
      "cropped": false,
      "size": 150
    },
    "lang": {
      "en": {
        "APPLY": "Apply",
        "CANCEL": "Cancel",
        "IMG": "Select image"
      },
      "ru": {
        "APPLY": "Сохранить",
        "CANCEL": "Отмена",
        "IMG": "Выберите картинку..."
      }
    },
    "init": function ($o,form) {
          var that = this,
              d = that.data;
          var html = $.my.formgen([
            '<div class="fl mr20 tac vat bg-lgray" style="width:600px;height:450px;'
            +'overflow:hidden;position:relative;line-height:450px;" id="crop-frame">',
            '<img id="source" class="vam" style="max-width:600px; max-height:450px; background:white" src="" />',
            '<div class="w600  dib" style="height:450px;position:absolute;top:0;left:0">',
            '<span class="dib vam fs110 button">',
            '<span class="fi-photo"><span class="fs110"> &nbsp;{IMG}</span></span>',
            '</span>',
            '<input type="file" id="file" class="w600 dib" accept="image/jpeg,image/png" '
            +'style="height:450px;cursor:pointer;opacity:0; position:absolute;top:0;left:0">',
            '</div>',
            '</div>',
            '<div class="w150 dib vat" id="xpreview">',
            '<canvas id="preview" class="bg-lgray" style="width:150px;height:150px" width='
            +form.data.size+' height='+form.data.size+' style="overflow:hidden;"></canvas>',
            {row:"150px",rowCss:"fs90 tac"},
            ["","btn#btn-apply.mb5.green.w150",{val:"{APPLY}"}, "btn#btn-close.mr0.w150",{val:"{CANCEL}"}],
            '</div>',
            '<div class="w150 dib vat" id="xwarn">',    
            '</div>',
            '<div class="hide"><canvas id="x2" width='+form.data.size*2+' height='+form.data.size*2+'></canvas></div>'
          ]);
          $o.html(html.assign(that.lang));
        
      },
    "style": {
      " .jcrop-holder": "display:inline-block;vertical-align:middle"
    },
    "ui": {
      "#file": {
        "bind": function (d,v,$o) {
                  var f,
                      $r = $o.my().root,
                      $c = $r.find("#preview"),
                      $img = $r.find("#source"),
                      $c2 = $r.find("#x2"),
                      n={k:1}, img="";
                  if (v!=null && v) {
                    f = $o[0].files[0];
                    if (f) (function(){
                      d.filename = f.name;
                      var fr = new FileReader(), ri, sb64 = [];
                      fr.onload=function(e){
                        ri = new Uint8Array(e.target.result);
                        for (var i=0; i<ri.length; i++) sb64.push(String.fromCharCode(ri[i]));
                        img = window.btoa(sb64.join(""));
                      };
                      fr.onloadend = function (){
                        $img.load(function(){
                          n.k = $img[0].naturalWidth/$img.width();
                        });
                        $img.removeClass("hide")
                        .attr("src","data:image/jpeg;base64,"+img);
                        n.k=$img[0].naturalWidth/$img.width();  
                        $img.Jcrop({
                          onChange: preview,
                          onSelect: preview.debounce(20),
                          aspectRatio: 1,
                          allowMove:true
                        }, function (){
                          this.animateTo([0,0,400,400]);
                        });
                        img="";
                        $o.parent().addClass("hide");
                      };
                      fr.readAsArrayBuffer(f);  
                    })();
                  }
                  function preview (c){
                    if(parseInt(c.w) > 0) {
                      // Show image preview
                      var img = $img[0], cs, ctx, cs2, ctx2, 
                          k = n.k,
                          w = c.w*k; //width to scale in 200px
                      if (w>d.size*2) {
                        // double downsample to remove
                        // bilinear-scaler artifacts
                        cs2 = $c2[0];
                        ctx2 = cs2.getContext("2d");
                        ctx2.fillStyle="white";
                        ctx2.fillRect(0,0,d.size*2, d.size*2);
                        ctx2.drawImage(img, c.x*k, c.y*k, w, w, 0, 0, d.size*2, d.size*2);
                        cs = $c[0];
                        ctx = cs.getContext("2d");
                        ctx.drawImage(cs2, 0, 0, d.size*2, d.size*2, 0, 0, d.size, d.size);
                      } else {
                        cs = $c[0];
                        ctx = cs.getContext("2d");
                        ctx.fillStyle="white";
                        ctx.fillRect(0,0,d.size, d.size);
                        ctx.drawImage(img, c.x*k, c.y*k, w, w, 0, 0, d.size, d.size);
                      }
                      d.cropped=true;
                    }
                  }
                
          }
      },
      "#btn-apply": {
        "bind": function (d,v,$o) {
                  if (v!=null && d.cropped) {
                    d.data = $o.my().root.find("#preview")[0]
                    .toDataURL('image/jpeg',0.93)
                    .substr(23);
                    $o.my().root.trigger("commit");
                  }
                
          },
        "events": "click.my"
      },
      "#btn-close": {
        "bind": function (d,v,$o) {
          if (v!=null) $o.my().root.trigger("cancel");
          },
        "events": "click.my"
      }
    }
  },
  "HTML": ["<div id=\"hheader\" class=\"bbs\"></div>",
    "<div id=\"cw-reg\" class=\"w550 vat dib tal xgray\" style=\"margin:0px auto 100px auto\">",
    "<div id=\"left\" class=\"w190 dib vat pr40\">",
    {
      "row": "150px",
      "rowCss": "my-row pb10 tac"
    },
    "<div class=\"slide\">",
    "<img id=\"pic\" src=\"\" style=\"width:150px;height:150px;cursor:pointer\" class=\"db mb5 br2\">",
    "<div id=\"pics\" class=\"mb10\"></div>",
    ["",
      "inp#name.w150.fs120.tac",
      {
        "plc": "{PLC_LOGIN}",
        "title": "{TIP_LOGIN}",
        "autocorrect": "off",
        "autocapitalize": "off",
        "autocomplete": "off"
      },
      "msg"],
    ["",
      "pwd#pin.w150.fs120.tac",
      {
        "plc": "{PLC_PIN}",
        "title": "{TIP_PIN}",
        "autocorrect": "off",
        "autocapitalize": "off",
        "autocomplete": "off"
      }],
    ["",
      "btn#btn-create.fs100.tac.mt10.mb5.br100",
      {
        "val": "{BTN_CREATE}"
      }],
    ["",
      "<div class=\"pb15 tac fs90 o70\"><span id=\"btn-logout\" class=\"pseudolink tdn\">{BTN_LOGOUT}</span></div>"],
    "</div>",
    "<div class=\"slide\" style=\"display:none\">",
    ["",
      "txt#dburl.w150.fs90.pb5",
      {
        "rows": 5,
        "plc": "{PLC_DBURL}"
      },
      "msg"],
    ["",
      "inp#dblogin.w150.fs110.tac",
      {
        "plc": "{PLC_DBLOGIN}",
        "autocorrect": "off",
        "autocapitalize": "off",
        "autocomplete": "off"
      }],
    ["",
      "pwd#dbpwd.w150.fs110.tac",
      {
        "plc": "{PLC_DBPWD}",
        "autocorrect": "off",
        "autocapitalize": "off",
        "autocomplete": "off"
      }],
    ["",
      "btn#btn-connect.w130.fs100.tac.mt10.mb20.br100",
      {
        "val": "{BTN_CONNECT}"
      },
      "<br>",
      "spn.btn-link.pseudolink.ml10.fs80",
      {
        "txt": "{BTN_NEWU}"
      }],
    "</div>",
    "</div>",
    "<div id=\"right\" class=\"w350 dib vat\">",
    "<div class=\"fs90 xgray slide\" id=\"about\">",
    "<div class=\"w350\" id=\"abouttext\"></div>",
    "</div>",
    "<div class=\"fs90 gray slide\" style=\"display:none\" id=\"aboutLink\"></div>",
    "</div>",
    "</div>"],
  "style": {
    " h3": "margin:20px 0 12px 0;",
    " p": "margin: 0.2em 0 0.7em 0;",
    "": "font-size:18px; line-height:25px;",
    " .hw28": "height:28px;width:28px;",
    " #hheader": {
      "": "padding:0px 20px 17px 0;margin:0 0 24px 0; text-align:left;width:100%;",
      " .hub-logo": "max-width:44px;max-height:33px; display:inline-block; vertical-align:middle;",
      " .hub-title": "max-width:calc(100% - 44px); font-size:1.76em; line-height:1.3em; padding:0 0 0 9px;display:inline-block; vertical-align:middle;  overflow:hidden; text-overflow:ellipsis;"
    },
    "#cw-reg": "width:530px;"
  },
  "files": {
    "0.png": {
      "digest": "md5-cJHlofu3xXv0gNoSzbjQ6w==",
      "content_type": "image/png",
      "revpos": 87,
      "data": ""
    },
    "1.png": {
      "digest": "md5-O0fW36XtR6hqU2Hql2aq8Q==",
      "content_type": "image/png",
      "revpos": 87,
      "data": ""
    },
    "2.png": {
      "digest": "md5-bdkheeCU4KhCK220aNFhRw==",
      "content_type": "image/png",
      "revpos": 87,
      "data": ""
    },
    "3.png": {
      "digest": "md5-f93LlaPKibYgqE81Hdq/2w==",
      "content_type": "image/png",
      "revpos": 87,
      "data": ""
    },
    "4.png": {
      "digest": "md5-xXtbGbTRRPxD9M9rRsquDg==",
      "content_type": "image/png",
      "revpos": 87,
      "data": ""
    }
  }
},
            {iscouch:false, doc: res }
          );
        })
        .fail(function (msg){
          pi.reject(500, 'Failed to start local profile editor.')
        });
      }

      return pi.promise();
    }

    // - - - - - - - - -

    function _startCW(){
      // Start CloudWall
      var pi = $.Deferred();
      $('#cw-page #cw-space').html(
        '<div class="p100">Starting up...<br><div class="cw-busy w90 dib vat mt10 mr5 h3"></div></div>'
      );
      cw.start().then(function (a){
        pi.resolve();
        console.log ('=== CloudWall 2.4.3 static ===');
      }, function (e, msg) {
        pi.reject(500, 'Startup sequence reported: '+e);
      });
      return pi.promise();
    }

    // - - - - - - - - - 

    function _req (url, opts) {
      var pi = $.Deferred();
      $.ajax($.extend(true, {
        url: cw.root + url,
        method:'GET',
        dataType:'json',
        headers:{accept:'application/json'},
        cache:false
      }, opts  || {}))
        .then(function(res){ pi.resolve(res); })
        .fail(function(res, b){
        var data;
        try {data = JSON.parse(res.responseText);} catch(e){}
        if (!Object.isObject(data)) data = {};
        data.ok = false;
        var code = res.status,
            msg = data.reason || data.message || '';
        pi.reject (code, msg, data);
      });
      return pi.promise();
    }
  };

})();